...
const uint32_t DELTA = 0x57E89147;
const int fillnor = 0xF8;

void XTea::NewCipher(uint32_t *key, std::vector<char> value)
{
	TeaCipher cipher;
	cipher.keys = key;
	cipher.ibuf = value;
...
inline void XTea::Push_bigendian(uint32_t *t, char *s)
{
	t[0] = *(s + 3) & 0x000000ff;
...
inline void XTea::Push_vec(std::vector<char> &t, uint32_t *s)
{
...
{
	char n = 32;
	uint32_t y = tc.ibyte32[0], z = tc.ibyte32[1], sum = DELTA << 5 & 0xffffffff;
	uint32_t a = tc.keys[0], b = tc.keys[1], c = tc.keys[2], d = tc.keys[3];
	while (n-- > 0) {
		z -= (((y << 4 & 0xFFFFFFF0) + c) ^ (y + sum) ^ ((y >> 5 & 0x07ffffff) + d));
		z &= 0xffffffff;
...
inline void XTea::M_xor(uint32_t *o, uint32_t *a, uint32_t *b)
{
	o[0] = a[0] ^ b[0];
	o[1] = a[1] ^ b[1];
}
